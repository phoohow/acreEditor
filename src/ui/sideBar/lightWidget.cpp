...
void LightWidget::setLight(rdr::LightID id)
{
    m_lightID = id;
...
void LightWidget::initUI()
{
    ...
    m_comboBox_type->addItem("Directional");
...
void LightWidget::updateProperties()
{
    ...
    switch (m_light->type)
    {
        case rdr::LightType::Direction:
            m_type = LightType::Directional;
            m_comboBox_type->setCurrentIndex(Directional);
            showDirectionUI();
            hideHDRUI();
            hidePointUI();
            break;
        case rdr::LightType::Point:
...
void LightWidget::showDirectionUI()
{
    ...
    auto factor = m_light->factor;
    auto dir    = m_light->direction;
...
void LightWidget::onUpdateColor()
{
    ...
    auto rgb = rdr::math::float3(r.toFloat(), g.toFloat(), b.toFloat());
...
void LightWidget::onUpdateDirection()
{
    ...
    m_light->direction = rdr::math::float3(x.toFloat(), y.toFloat(), z.toFloat());
    m_scene->updateLight(m_lightID);
    m_flushFrame();
}
...
void LightWidget::onUpdatePosition()
{
    ...
    m_light->position = rdr::math::float3(x.toFloat(), y.toFloat(), z.toFloat());
    m_scene->updateLight(m_lightID);
    m_flushFrame();
}
