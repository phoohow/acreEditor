...
{
#if USE_VULKAN
    m_deviceMgr = rdr::createDeviceMgr(rdr::DeviceType::rVulkan);
#else
    m_deviceMgr = rdr::createDeviceMgr(rdr::DeviceType::rDX12);
#endif

#ifndef _DEBUG
    m_renderScene = rdr::createScene(m_deviceMgr, QDir::currentPath().toStdString().c_str());
#else
    auto srcDir   = SRC_DIR;
    auto dstDir   = DST_DIR;
    m_renderScene = rdr::createScene(m_deviceMgr, srcDir, dstDir);
#endif
...
RenderWindow::~RenderWindow()
{
    rdr::destoryRenderer(m_renderer);
    rdr::destoryScene(m_renderScene);
    rdr::destorySwapchain(m_swapchain);
    rdr::destroyDeviceMgr(m_deviceMgr);
}
...
void RenderWindow::keyPressEvent(QKeyEvent* event)
{
    ...
    {
        case Qt::Key_A:
            m_scene->cameraMove(rdr::math::float3(1.0f, 0, 0));
            break;
        case Qt::Key_D:
            m_scene->cameraMove(rdr::math::float3(-1.0f, 0, 0));
            break;
        case Qt::Key_W:
            m_scene->cameraMove(rdr::math::float3(0, 0, 1.0f));
            break;
        case Qt::Key_S:
            m_scene->cameraMove(rdr::math::float3(0, 0, -1.0f));
            break;
        case Qt::Key_Q:
            m_scene->cameraMove(rdr::math::float3(0, 1.0f, 0));
            break;
        case Qt::Key_E:
            m_scene->cameraMove(rdr::math::float3(0, -1.0f, 0));
            break;
